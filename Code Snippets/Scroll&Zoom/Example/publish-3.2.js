		...
		if(child[i].attributes != null){
			if (child[i].getAttribute('class') == 'area ui-content') {
				if(navigator.userAgent.indexOf('iPhone')<0){
					...
					img.src = './watertowninfo_files/zoomIn.png';
					img.id = 'mapZoomIn';
					var page = document.getElementById('image-8');
					page.appendChild(img);
					//img.setAttribute('onclick','alert(\'In\');zoomInMap();');
					$(img).css('top','0px');
					...
					img1.src = './watertowninfo_files/zoomOut.png';
					img1.id = 'mapZoomOut';
					var page = document.getElementById('image-8');
					page.appendChild(img1);
					//img1.setAttribute('onclick','alert(\'Out\'); zoomOutMap();');
					$(img1).css('top','0px');
				...
				$(scrollView).css('height', imgHeightConstant+ 'px');
...
function loadEventLister(){
	...
	/*
var deviceType = window.navigator.appVersion;
		swipeWorking = true;
		if ((deviceType.indexOf('iPhone') > 0) || (deviceType.indexOf('iPad') > 0) || (deviceType.indexOf('Android') > 0)) {
			...
			$(canvas[0]).bind('click',function(event){event.preventDefault();});
		}
...
function zoomInMap(){
		...
		var mapImg = document.getElementById('pinchListener');
		if(specialZoom>=8){
			return;
		}
